 ...
 */
class Opc_Translate_Adapter_Yaml_sfYamlParser
{
  protected
    $value         = '',
    $offset        = 0,
    ...
    while ($this->moveToNextLine())
    {
      ...
      if (preg_match('#^\-(\s+(?P<value>.+?))?\s*$#', $this->currentLine, $values))
      {
        ...
        if (!isset($values['value']) || '' == trim($values['value'], ' ') || 0 === strpos(ltrim($values['value'], ' '), '#'))
        {
          ...
          $parser = new Opc_Translate_Adapter_Yaml_sfYamlParser($c);
        ...
        else
        {
          ...
          {
            $data[] = array($matches[1] => Opc_Translate_Adapter_Yaml_sfYamlInline::load($matches[2]));
          }
          else
          {
      ...
      else if (preg_match('#^(?P<key>'.Opc_Translate_Adapter_Yaml_sfYamlInline::REGEX_QUOTED_STRING.'|[^ ].*?) *\:(\s+(?P<value>.+?))?\s*$#', $this->currentLine, $values))
      {
        $key = Opc_Translate_Adapter_Yaml_sfYamlInline::parseScalar($values['key']);
      ...
      else
      {
        ...
        if (1 == count(explode("\n", rtrim($this->value, "\n"))))
        {
          $value = Opc_Translate_Adapter_Yaml_sfYamlInline::load($this->lines[0]);
          if (is_array($value))
          {
            $first = reset($value);
            if ('*' === substr($first, 0, 1))
  ...
  protected function parseValue($value)
  {
    ...
    {
      return Opc_Translate_Adapter_Yaml_sfYamlInline::load($value);
    }
  }
